...
# Sync contract test: run scripts/sync_fixture.py, start the app with SYNC_JIRA_ENABLED=true
# SYNC_JIRA_BASE_URL=http://127.0.0.1:18101 (+username/token, gitlab/zentao vars), then scripts/check_sync_contract.py
...
`auth`, `employee`, `skill`, `project`, `allocation`, `solver`, `ai`, `system`, `capability`, `integration`, `common`. `capability` is read-only analytics (`CapabilityService`: skill supply-demand gaps in flat or weekly-refined models, bottleneck-skill key people, leave-impact what-if, pipeline scenarios, gap trends, admin-imported market benchmarks, AI advice). CRUD modules follow `controller/dto/entity/mapper/service`; `solver`/`allocation`/`system` are flatter and use raw `JdbcTemplate`. `system` also hosts `NotifyService`: after-commit outbound webhooks (`PLAN_CONFIRMED`, `AVAILABILITY_CONFLICT`, `REPLAN_SUGGESTED`) to Feishu/DingTalk/WeCom/generic endpoints, logged in `notification_log` with masked targets. `allocation` also hosts `ReplanTriggerService` (`REPLAN_AUTO_TRIGGER=off|detect|auto`): after-commit async patrol of active plans on availability / skill-profile / skill / project-window changes; `auto` drafts a replan (human confirm required), audited in `replan_trigger_log`. New `integration` module: one-way external project sync (Jira REST v2 / ZenTao OpenAPI v1 / GitLab v4 → PLANNING projects, default sequential schedule, `integration_link` mapping, refresh blocked while allocations active; contract test `scripts/sync_fixture.py` + `scripts/check_sync_contract.py`).

- `skill` also hosts the optional pgvector semantic search (`SKILL_SEMANTIC_ENABLED`; extra Flyway location `db/migration-semantic` V900+; `local` deterministic hash or `live` /v1/embeddings; vector suggestions for unmatched names at 0.5 on top of the 0.82 deterministic hint) and `AiSkillProfileService`: AI skill-profile drafts from experience text or uploaded resume files (`ai-extract` / `ai-extract-file`; `demo` = deterministic `DemoSkillExtractor`, `live` = LLM + `SkillNormalizer` + `SkillSimilarity` hints at 0.82+), human-confirmed merges into `employee_skill` (`ai-accept`), and skill evidence with profile-vs-suggested levels (`skills/evidence`, one-click adoption). AI extracts; only humans confirm writes.
...
2. `PlanningRepository.load(projectId)` — assembles the solve snapshot: leaf tasks only (1–200), Mon–Fri workdays, project span ≤ 2 years, dependency types FS/SS/FF/SF validated against dates. Also computes `input_hash`: SHA-256 where project tables (`project`/`task`/`task_dependency`/`task_skill_requirement`) scope to this project while org tables (`employee`/`employee_skill`/`skill`/`employee_availability`/`resource_allocation`) stay whole — the optimistic-freshness guard; task creation in other projects no longer invalidates a draft.
...
4. Plan lifecycle: DRAFT → `edit()` (manual reassignment; re-checks over-allocation via `ResourceConstraints.excess`) → `confirm()` (re-checks input_hash, no gaps; `LOCK TABLE ... IN SHARE ROW EXCLUSIVE MODE`; atomically archives any previous active set of the same project, then copies items into `resource_allocation`) → `cancel()`. Base-data changes (leave / skill profile / project window) can auto-trigger replan drafts via `ReplanTriggerService` when enabled; drafts always require human confirmation. Exactly one active allocation set per project: with one active, plain `solve` is rejected — use `replan()` (re-solves with own bookings excluded), `GET .../replan/impact` (five conflict types), the org-wide patrol `GET /replan/alerts`, and `POST /resource-plans/{id}/explain-diff` (AI diff explanation for replan drafts) for the dynamic replan loop. Plan detail responses carry computed `gapSummary` and cross-project weekly `warnings` (load ≥80% incl. other projects' allocations), which also feed `review()`.
5. `AiPlanningService.review(planId)` — AI explains an already-solved plan; never modifies it.

Execution-side lifecycle (after allocations take effect): `POST /tasks/{id}/status`, `/projects/{id}/status` and `/employees/{id}/status` enforce pure-function state machines (package-private `canTransition`, unit-tested). Closing a task (DONE/CANCELLED) also closes its active bookings (allocation status COMPLETED/CANCELLED), freeing capacity everywhere at once since all consumers filter `PLANNED/CONFIRMED`; terminal projects (COMPLETED/CANCELLED) reject solving; deactivating an employee enqueues `EMPLOYEE_STATUS_CHANGED` patrols.
