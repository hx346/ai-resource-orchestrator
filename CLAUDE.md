...
4. Plan lifecycle: DRAFT → `edit()` (manual reassignment; re-checks over-allocation via `ResourceConstraints.excess`) → `confirm()` (re-checks input_hash, no gaps, no conflicting allocations; `LOCK TABLE ... IN SHARE ROW EXCLUSIVE MODE`; copies items into `resource_allocation`) → `cancel()`. Only one active allocation set per project. Plan detail responses carry computed `gapSummary` and cross-project weekly `warnings` (load ≥80% incl. other projects' allocations), which also feed `review()`.
