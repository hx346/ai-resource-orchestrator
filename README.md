...
- 需求 = 预测窗口内未完成任务（含已分配，供给同步扣减占用）的技能需求工时；达标线取该技能在任务需求中的**最高** min_level
- 供给 = 达标且在职员工的窗口可用工时：工作日 × 8h × 默认容量% − 生效分配占用 − 不可用窗口（休假/出差等）
- 缺口人数按 **0.8 有效投入系数**折算；建议区分为「无人达标（招聘/外包）」与「有人但容量不足（调配/培训/招聘）」
...
「资源排期」页已提供按周聚合的全员 Capacity Timeline / Heatmap（27 周视野，绿/黄/红三档负荷，悬停查看项目占用明细）；项目详情页提供按日的任务甘特图（周末底纹、优先级配色、里程碑标记）。
